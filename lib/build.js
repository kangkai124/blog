module.exports = async function build (sourceDir, cliOptions = {}) {
  ...
  const noteDir = path.resolve(__dirname, '../../imkk.asia')
  logger.success(`\n${chalk.green('Success!')} Generated static files in ${chalk.cyan(relativeDir)}.\n`)
  logger.wait('将打包后的文件复制到 note 文件夹')
  try {
    await fs.copy(relativeDir, noteDir)
    cd(noteDir)
    exec('git add .')
    exec(`git commit -m 'update at ${new Date()}'`)
    exec('git push')
    logger.success('更新到 github ...')
  } catch (err) {
    logger.error(err)
  }
